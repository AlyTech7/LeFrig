...
export function ExclusiveAdsCarousel() {
  ...
  const go = useCallback((i: number) => setActive((i + ADS.length) % ADS.length), []);
  ...
  return (
    <section className="lf-promo" aria-label={t('promo.label')}>
      <p className="lf-promo__label">
        <span>{t('promo.sponsored')}</span>
      </p>
      ...
      <div className="lf-promo__card">
        {ads.map((item, i) => (
          ...
          >
            ...
            <Link href={item.href} className="lf-promo__btn" tabIndex={i === active ? 0 : -1}>
              {t('promo.view')}
            </Link>
          </article>
        ))}
